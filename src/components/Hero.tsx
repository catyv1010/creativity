...
function MagneticButton({ children, className, href = "/cinema-editor" }: { children: React.ReactNode; className?: string; href?: string }) {
  ...
  return <motion.a ref={ref} href={href} style={{ x: sx, y: sy }} onMouseMove={onMove} onMouseLeave={onLeave} className={className}>{children}</motion.a>;
}
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#030014]">
      {/* Radial lights */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(120,40,200,0.15)_0%,transparent_60%)]" />
      ...
      <motion.div style={{ x: sx, y: sy }} className="absolute inset-0 pointer-events-none">
        <div className="absolute w-[500px] h-[500px] -top-[10%] -left-[8%] rounded-full bg-purple/10 blur-[100px]" style={{ animation: "morphBlob1 18s ease-in-out infinite" }} />
        <div className="absolute w-[400px] h-[400px] top-[20%] -right-[5%] rounded-full bg-coral/10 blur-[100px]" style={{ animation: "morphBlob2 22s ease-in-out infinite" }} />
        <div className="absolute w-[300px] h-[300px] bottom-[5%] left-[25%] rounded-full bg-cyan/6 blur-[80px]" />
      </motion.div>

      {/* Geometric elements */}
      <motion.svg className="absolute top-[14%] left-[7%] w-20 h-20 opacity-20 pointer-events-none" viewBox="0 0 80 80" fill="none" animate={{ rotate: 360 }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }}>
        <circle cx="40" cy="40" r="35" stroke="#a855f7" strokeWidth="0.8" strokeDasharray="6 4" />
      </motion.svg>
      <motion.svg className="absolute top-[10%] right-[9%] w-28 h-28 opacity-15 pointer-events-none" viewBox="0 0 120 120" fill="none" animate={{ rotate: 360 }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }}>
        <rect x="20" y="20" width="80" height="80" rx="6" stroke="#F2583E" strokeWidth="0.8" />
      </motion.svg>
      <motion.svg className="absolute bottom-[22%] left-[6%] w-24 h-24 opacity-15 pointer-events-none" viewBox="0 0 100 100" fill="none" animate={{ y: [0, -15, 0] }} transition={{ duration: 10, repeat: Infinity }}>
        <polygon points="50,10 90,90 10,90" stroke="#f59e0b" strokeWidth="0.8" />
      </motion.svg>
      <motion.svg className="absolute bottom-[28%] right-[7%] w-20 h-20 opacity-15 pointer-events-none" viewBox="0 0 90 90" fill="none" animate={{ scale: [1, 1.15, 1], rotate: [0, 60, 0] }} transition={{ duration: 12, repeat: Infinity }}>
        <polygon points="45,5 85,27 85,63 45,85 5,63 5,27" stroke="#06b6d4" strokeWidth="0.8" />
      </motion.svg>

      {/* Breathing dots */}
      {[{ p: "top-[30%] left-[18%]", c: "bg-purple/40" }, { p: "top-[55%] right-[22%]", c: "bg-coral/40" }, { p: "top-[70%] left-[60%]", c: "bg-cyan/30" }].map((d, i) => (
        <motion.div key={i} className={`absolute w-1.5 h-1.5 rounded-full ${d.c} ${d.p} pointer-events-none`}
          animate={{ scale: [1, 2.5, 1], opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 3 + i, repeat: Infinity, delay: i * 0.7 }} />
      ))}

      {/* Particles — fixed values to avoid hydration mismatch */}
      {[
        ...
        <div key={i} className="particle" style={{ left: `${p.l}%`, bottom: "-3%", width: p.w, height: p.h, background: ["#a855f7", "#ec4899", "#06b6d4", "#f59e0b", "#F2583E"][i % 5], opacity: p.o, animationDelay: `${p.dd}s`, animationDuration: `${p.du}s` }} />
      ))}

      {/* CONTENT */}
      <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8 }}
        ...
        <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.8, duration: 0.8 }}
          ...
          <MagneticButton className="hoverable group relative px-10 py-5 rounded-2xl bg-white text-black font-extrabold text-lg overflow-hidden inline-block">
            <span className="relative z-10 flex items-center gap-3 transition-colors duration-500 group-hover:text-white">
              ...
              <svg className="w-5 h-5 group-hover:translate-x-2 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
            </span>
      ...
      {/* Scroll */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.5 }} className="absolute bottom-10 left-1/2 -translate-x-1/2">
